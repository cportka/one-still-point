 ...
 * two; a natural inspiral crosses r ≈ 20 (comfortably above the Roche radius) well before tearing.
 * Firing on those signals moves the freeze to a moment when the body is drifting gently — barely
 * perceptible — and the drama itself plays on the already-compiled shader. The scaler is frozen
 * during the compile (the depth-texture crash guard) and its smoothing reset after, so the giant
 * compile frame can't crater the resolution either.
 */
export const FULL_SHADER_APPROACH_R = 20;
